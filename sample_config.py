...
class Config(object):
    TG_BOT_TOKEN = os.environ.get("TG_BOT_TOKEN", "8369915876:AAH9wJ1yOotFTjdTrRzJlW2u480QNOoP9SQ") # Make a bot from https://t.me/BotFather and enter the token here
    #If deploying on vps edit the above value as example := TG_BOT_TOKEN = "Your-bot-token-inside-inverted-commas."
    
    APP_ID = int(os.environ.get("APP_ID", 34446649)) # Get this value from https://my.telegram.org/apps
    #If deploying on vps edit the above value as example := APP_ID = Your-APP_ID-without-inverted-commas
    
    API_HASH = os.environ.get("API_HASH", "8dc570c08d8e35e88fb9bfc73c65d7fa") # Get this value from https://my.telegram.org/apps
    ...
    Bot_username = os.environ.get("Bot_username", "@unzipper_ak_bot") # Your bot's telegram username (must enter with '@' in the front of the username)
    #If deploying on vps edit the above value as example := Bot_username = "Your-Bot_username-inside-inverted-commas."
    
    OWNER_ID = os.environ.get("OWNER_ID", "7892805795") # Your(owner's) telegram id
    ...
    REDIS_PASS = os.environ.get("REDIS_PASS", "cbHQ1TMyVEI4eui") # Get This Value from http://redislabs.com/try-free (If you don't know how to obtain the a video tutorial is available here:- https://t.me/botzupdate/5)
    #If deploying on vps edit the above value as example := REDIS_PASS = "Your-Redis-Password-inside-inverted-commas."

    AUTH_USERS = set(int(x) for x in os.environ.get("AUTH_USERS", "7892805795").split()) # Id's of the telegram users, who you want to allow for multitasking - downloading multiple links at once!
